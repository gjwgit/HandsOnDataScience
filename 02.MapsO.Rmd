...
p <- ggmap(map)
p <- p + geom_text(data=lbl, aes(x=lon, y=lat, label=text),colour='blue', vjust=1.5)
p <- p + geom_point(data=lbl, aes(x=lon, y=lat),colour='red', size=3)
p
```
...
leaflet() %>% addTiles() %>% addMarkers(data=smpl,lng=smpl$coords.x1,
                                        lat=smpl$coords.x2)

...
```{r,echo=FALSE}
meta <- read.csv(file = 'tables/metacharacters.csv',header = T)
knitr::kable(
   meta,
...
```{r,message=F,metacharacterEg,warning=F}
...
quantifier <- read.table(file = 'tables/quantifier.csv',header = T,sep = '\t')
knitr::kable(
   quantifier,
...
grep(pattern = '[[:alpha:]]',string,value = T)
#Matching upper case characters
grep(pattern = '[[:upper:]]',string,value = T)
```
...
```{r,echo = F}
regex_func <- read.csv('tables/regex_functions.txt',sep = ',',header=T)
knitr::kable(
  regex_func,
